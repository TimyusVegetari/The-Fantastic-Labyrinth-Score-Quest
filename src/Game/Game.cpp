...
Game::Game ( void ) {
  ...
  m_v2uSize         = sf::Vector2u (THEFANTASTICLABYRINTH_WIDTH, THEFANTASTICLABYRINTH_HEIGHT);
  m_szTitle         = THEFANTASTICLABYRINTH_NAME;
  m_szAuthor        = THEFANTASTICLABYRINTH_AUTHOR;
  m_szVersion       = THEFANTASTICLABYRINTH_VERSION;
  m_szCopyright     = THEFANTASTICLABYRINTH_LICENSE;
  m_szDate          = THEFANTASTICLABYRINTH_DATE;
  m_uiTickDelay     = 0;
  m_uiTickTrigger   = 0;
...
GLboolean Game::GameInit ( void ) {
  ...
  std::cout << "==== Loading of the tiles ==========" << std::endl;
  // Loading of the tiles
  m_oTilesManager.Initialize (m_oGameObjectsManager.GetContext ());
  std::cout << "" << std::endl;

  std::cout << "==== Loading of the high scores ==========" << std::endl;
  // Loading of the high scores
  m_oScoreBoard.LoadHighScores ();
  std::cout << "" << std::endl;

  std::cout << "==== Loading of the foods ==========" << std::endl;
  // Loading of the foods
  m_oFoodsManager.Initialize (m_oGameObjectsManager.GetContext ());
  std::cout << "" << std::endl;
...
void Game::GameCycle ( void ) {
  ...
  if (TickClock ()) {
    ...
    SetScreenColor (m_oGraphicsEngine.GetScreenColor ());
