...
sf::Vector2f BmpText::FindCharacterPos ( std::size_t index ) const {
  ...
  if (m_sfString.getSize () > 0) {
    ...
    GLfloat fHSpace = static_cast<GLfloat> (m_pBmpFont->GetKerning (L' ', bBold));
    GLfloat fVSpace = static_cast<GLfloat> (m_pBmpFont->GetLineSpacing ());

    // Compute the position
    sf::Uint32 uiPrevChar = m_sfString[0];
    for (std::size_t i=1 ; i<=index ; ++i) {
      sf::Uint32 uiCurChar = m_sfString[i];

      // Handle special characters
      switch (uiPrevChar) {
        case L'\t'  : sfPosition.x += fHSpace * 2.f;                break;
        case L'\n'  : sfPosition.y += fVSpace; sfPosition.x = 0.f;  break;
        case L'\v'  : sfPosition.y += fVSpace * 2.f;                break;
        // For regular characters, apply the kerning offset and add the advance offset of the glyph
        default     : sfPosition.x += static_cast<GLfloat> (m_pBmpFont->GetKerning (uiPrevChar, bBold))
                                    + static_cast<GLfloat> (m_uiGlyphAdvance);
      }
